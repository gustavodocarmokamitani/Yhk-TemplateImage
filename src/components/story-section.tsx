...
function StoryChapterCard({
  ...
  reversed: boolean;
  onOpenPhoto: (chapterId: string, photoId: string) => void;
}) {
  const ref = useRef<HTMLElement>(null);
  // The photo grid drifts a few pixels against the scroll while the text
  // stays put, so the two read as separate depth planes instead of one flat
  // block — the cheapest version of a parallax effect that still reads at a
  // glance.
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "end start"],
  });
  const photosY = useTransform(scrollYProgress, [0, 1], [32, -32]);
  ...
  return (
    ...
    >
      <motion.div variants={fadeInUp} className="sm:flex-1">
        <h2 className="text-2xl font-medium tracking-wide text-white sm:text-3xl">{chapter.title}</h2>
        {chapter.dateLabel && (
      ...
      >
        {chapter.photos.map((photo) => (
          <button
            key={photo.id}
            type="button"
            onClick={() => onOpenPhoto(chapter.id, photo.id)}
            className="group relative aspect-[4/5] overflow-hidden rounded-lg bg-[#121212] transition-shadow duration-500 hover:shadow-2xl hover:shadow-black/50"
          >
            <GradedImage
              src={photo.thumbUrl}
